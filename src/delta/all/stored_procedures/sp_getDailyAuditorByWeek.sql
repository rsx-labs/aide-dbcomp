...
WHILE @cnt < 5
		BEGIN
			SET @GETPERDATESTRING = CONCAT(MONTH(DATEADD(day, @cnt, @DATEVALUE)),'/',DAY(DATEADD(day, @cnt, @DATEVALUE)))  --CONCAT(CONVERT(MONTH( DATEADD(day, @cnt, @DATEVALUE), 1), 0, 6) --CONCAT(MONTH(@DATEVALUE), '/', DATENAME(DAY,DATEADD(wk,	0, DATEADD(DAY, @cnt-DATEPART(WEEKDAY, @DATEVALUE), DATEDIFF(dd, 0, @DATEVALUE)))))
			SET @SETdatechecked = CONVERT(varchar, DATEADD(day, @cnt, @DATEVALUE), 23) --CONCAT(YEAR(@DATEVALUE), '-',month(@DATEVALUE),'-' ,DATENAME(DAY,DATEADD(wk,	0, DATEADD(DAY, @cnt-DATEPART(WEEKDAY, @DATEVALUE), DATEDIFF(dd, 0, @DATEVALUE)))))
	
		INSERT INTO #SummaryDailyAuditor
				SELECT SUBSTRING(DATENAME(WEEKDAY,  DATEADD(day, @cnt, @DATEVALUE)), 1, 3), -- DISTINCT SUBSTRING(DATENAME(weekday,DATEADD(wk,	0, DATEADD(DAY, @cnt-DATEPART(WEEKDAY, @DATEVALUE), DATEDIFF(dd, 0, @DATEVALUE)))), 1, 3)	,
					E.FIRST_NAME , 
					E.EMP_ID , 
					@GETPERDATESTRING,
					WAD.FY_WEEK,
				 ...
				 FROM WORKPLACE_AUDIT_SCHEDULE WAS 
					...
					WHERE was.FY_WEEK = @FY_WEEK --MONTH(WAS.PERIOD_START) = MONTH(@DATEVALUE) and was.FY_WEEK = @FY_WEEK
...
SELECT * FROM #SummaryDailyAuditor2 ORDER BY WEEKDATESCHED ASC --where MONTH(WEEKDATESCHED) = MONTH(@DATEVALUE) ORDER BY WEEKDATESCHED ASC
END
